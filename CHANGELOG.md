...
- `vue-error-boundary-kit/retry-backoff` — `createBackoffRetry(boundaryRef, options?)`: wraps a `<ErrorBoundary>`/`<AsyncBoundary>` template ref's `retry()` with an increasing delay (`baseDelayMs * factor ** retryCount`, capped at `maxDelayMs`) for transient failures where an instant retry just fails again the same way. Kept out of `<ErrorBoundary>`'s own props deliberately — not every use case wants delayed retry, and this composes with the existing exposed `{ retry, retryCount }` API instead of growing it
